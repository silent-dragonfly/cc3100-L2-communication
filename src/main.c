...
const int TRY_RECIVE_MAX = 15;
const ROLE_OFFSET = 25;
const COUNTER_OFFSET = 26;

DataQoSFrame_FromDSToSTA FRAME = { .FrameControl = (struct FrameControl ) {
				.Type = TYPE_DATA, .Subtype = DATA_SUBTYPE_QOS, .FromDS = 1 },
		.Duration = 0x002C, .DestinationMAC = { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
				0xAA }, .BSSIDMAC = { 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB },
		.SourceMAC = { 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB }, .SequenceControl =
				(struct SequenceControl ) { .FragmentNumber = 0,
								.SequenceNumber = 0x428, }, .QoSControl =
				(struct QoSControl ) { 0 }, .Data = {
		/* LLC */
		0xAA, // DSAP
				0xAA, // SSAP
				0x03, // Control field
...
typedef enum AppRole {
	APP_PINGER = 0x55, APP_PONGER = 0xDD
} AppRole;
...
static _i32 main_pinger() {
	...
	for (int counter = 0; counter < END_COUNT;) {
		...
		if (Status < 0) {
			DEBUG("[ERROR] while sending packet: %d", Status);
			sl_Close(SockID);
			return -1;
		}
		...
		for (int rx_counter = 0; rx_counter < TRY_RECIVE_MAX; rx_counter++) {
			...
			DataQoSFrame_FromDSToSTA *recv_fr =
					(DataQoSFrame_FromDSToSTA*) (BUFFER
							+ sizeof(SlTransceiverRxOverHead_t));
			FrameControl *fc = &recv_fr->FrameControl;

			if (Status < 0) {
				DEBUG("[ERROR] while receiving packet: %d", Status);
				sl_Close(SockID);
				return -1;
			}
			...
			if (recv_fr->Data[ROLE_OFFSET] != APP_PONGER) {
				DEBUG("[PINGER] Unexpected role of sender: %d",
						recv_fr->Data[ROLE_OFFSET]);
				continue;
			}

			if (recv_fr->Data[COUNTER_OFFSET] != counter) {
				DEBUG("[PINGER] Unexpected counter in response: %d",
						recv_fr->Data[COUNTER_OFFSET]);
				continue;
			}

			DEBUG("[%lu]Recv: %d | %d bytes; ch %u; rate: %u; rssi %d",
					recv_fr->Data[COUNTER_OFFSET], rxHeader->timestamp, Status,
					rxHeader->channel, rxHeader->rate, rxHeader->rssi);
...
static _i32 main_ponger() {
	...
	while (recv_counter != (END_COUNT - 1)) {
		...
		for (int rx_counter = 0; rx_counter < TRY_RECIVE_MAX; rx_counter++) {
			...
			if (recv_fr->Data[ROLE_OFFSET] != APP_PINGER) {
				DEBUG("[PONGER] Unexpected role of sender: %d",
						recv_fr->Data[ROLE_OFFSET]);
				continue;
			}
		...
		if (!RESP_RECEIVED) {
			DEBUG("[WARNING] not receive answer from PINGER. recv_counter %d",
					recv_counter);
		}
	...
	DEBUG(
			"[PONGER] Sending finishing 20 packets to give chance PINGER to finish");
	for (int i = 0; i < 20; i++) {
		Status = sl_Send(SockID, (_u8*) &FRAME, sizeof(FRAME),
				SL_RAW_RF_TX_PARAMS(CHANNEL, RATE, POWER_LEVEL_TONE, PREAMBLE));
		if (Status < 0) {
